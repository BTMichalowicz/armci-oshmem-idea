...
AC_DEFUN([PAC_LIB_OSHMEM],[
dnl Set the prereq to 2.50 to avoid having 
AC_PREREQ(2.50)
if test "X$pac_lib_oshmem_is_building" != "Xyes" ; then
  ...
  AC_SEARCH_LIBS(shmem_init,$shmemlib shmem oshmem)
  if test "$ac_cv_search_oshmem_init" = "no" ; then
    ifelse($2,,
    AC_MSG_ERROR([Could not find OSHMEM library]),[$2])
  fi
  AC_CHECK_HEADER(shmem.h,pac_have_shmem_h="yes",pac_have_shmem_h="no")
  if test $pac_have_shmem_h = "no" ; then
    ifelse($2,,
    AC_MSG_ERROR([Could not find shmem.h include file]),[$2])
  fi
  if test "X$pac_save_level" != "X" ; then
     CC="$pac_save_TESTCC"
...
dnl D*/
AC_DEFUN([PAC_ARG_OSHMEM_TYPES],[
PAC_ARG_OSHMEM_KNOWN_TYPES
PAC_OSHMEM_FIND_COMPILERS
PAC_OSHMEM_CHECK_MPI_LIB
])
dnl
...
dnl
AC_DEFUN([PAC_ARG_OSHMEM_KNOWN_TYPES],[
AC_ARG_WITH(mpich,
...
AC_ARG_WITH(oshmem,
[--with-shmem=path  - Use an OpenSHMEM implementation with compile scripts oshcc
                     and oshcxx in path/bin],ac_oshmem_type=generic)

if test "X$ac_oshmem_type" = "X" ; then
    if test "X$1" != "X" ; then
        ac_oshmem_type=$1
    else
        ac_oshmem_type=unknown
    fi
fi
if test "$ac_oshmem_type" = "unknown" -a "$pac_lib_oshmem_is_building" = "yes" ; then
    ac_oshmem_type="osss-ucx"
fi
])
...
AC_SUBST(MPIEXEC_N)
dnl
AC_ARG_VAR([OSHRUN],[Name and path of oshrun program])
...
changequote([,])
    ...
    ;;  
  *)
    AC_MSG_ERROR([OSHCC must be set to an absolute path if it is set])
  esac
fi
if test -n "$OSHCXX" ; then
   case $OSHCXX in
changequote(<<,>>)
    [\\/]* | ?:[\\/]*)
changequote([,])
    ...
    ;;  
  *)
    AC_MSG_ERROR([OSHCXX must be set to an absolute path if it is set])
  esac
fi
...
AC_DEFUN([PAC_OSHMEM_CHECK_OSHMEM_LIB],[
case $ac_oshmem_type in
	mpich)
	;;
...
dnl D*/
dnl AC_DEFUN([PAC_MPI_F2C],[
dnl AC_CACHE_CHECK([for MPI F2C and C2F routines],
dnl pac_cv_mpi_f2c,
dnl [
dnl AC_TRY_LINK([#include "mpi.h"],
dnl [MPI_Request request;MPI_Fint a;a = MPI_Request_c2f(request);],
dnl pac_cv_mpi_f2c="yes",pac_cv_mpi_f2c="no")
dnl ])
dnl if test "$pac_cv_mpi_f2c" = "yes" ; then 
dnl    AC_DEFINE(HAVE_MPI_F2C,1,[Define if MPI has F2C]) 
dnl fi
dnl ])
dnl
dnl/*D
...
dnl D*/
dnl AC_DEFUN([PAC_HAVE_ROMIO],[
dnl if test "$enable_romio" = "yes" ; then HAVE_ROMIO='#include "mpio.h"'; fi
dnl AC_SUBST(HAVE_ROMIO)
dnl ])
